...
IloExpr Expr15(env);
  ...
  for ( t = 0 ; t < T ; t++ ){
    ...
    throw(-1);
        ...
        //Cópia do vetor dos lucros
        int vetor[N];
        IloNumArray VetorLucro (env, N);
...
//Ordena o vetor de lucros em forma decrescente
    for (i=0; i<N; i++){
       for(j=0; j<N; j++){
            if(VetorLucro[j] < VetorLucro[i] ){
                aux = VetorLucro[j];
                VetorLucro[j] = VetorLucro[i];
                VetorLucro[i] = aux;
                aux2 = vetor[j];
                vetor[j] = vetor[i];
                vetor[i]= aux2;
...
fase2.setParam(IloCplex::Param::TimeLimit, temporestante/3);
